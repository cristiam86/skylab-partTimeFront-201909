...
describe('RPS CORRECT SELECTED', function(){
  test('should select correct options', function(){
    var result = rockPaperScissorsLogic.play('hola', 'paper');
    expect(result).toBe('options incorrect selected');
  });
  test('should select correct options', function(){
    var result = rockPaperScissorsLogic.play('hola', 'rock');
    expect(result).toBe('options incorrect selected');
  });
  test('should select correct options', function(){
    var result = rockPaperScissorsLogic.play('hola', 'scissors');
    expect(result).toBe('options incorrect selected');
  });
})

describe('RPS CORRECT FORMAT', function(){
  test('should be correct format', function(){
    var result = rockPaperScissorsLogic.play('rock', 1);
    expect(result).toBe('incorrect format');
  });
  test('should be correct format', function(){
    var result = rockPaperScissorsLogic.play(1, 'paper');
    expect(result).toBe('incorrect format');
  });
})
